    ...
    var EventNamespace = (function (){
        var that = EventNamespace || {},
            ...
            customFilterSubmit = function (searchToken, searchFilter) {
                AjaxOperations
                    ...
                    .then(DomManipulations.wordList.populate, View.failDialog);
            }
        ;
        ...
        that.keyHandlers = {
            dismissViewers: function (event) {
                if (event.which === KEYCODES.VK_ESCAPE) {
                    View.resetWordEdit();
                    View.hideElement($('#wordEdit'));
                    View.resetWordView();
                    View.hideElement($('#wordView'));
                }
            },
        ...
        that.clickHandlers = {
            wordViewTrigger: function (event) {
                ...
                View.showWordView(event, wordSpelling);
            },
            wordView: function (event) {
                $(".selected").removeClass("selected");
                View.hideElement($(this));
                event.stopPropagation();
            },
            ...
            filters: function (event) {
                ...
                if (target.is('#allFilter')) {
                    AjaxOperations.getWords().then(DomManipulations.wordList.populate, View.failDialog);
                }
                    ...
                    AjaxOperations.getWords(letterVal, "starts").then(DomManipulations.wordList.populate, View.failDialog);
                }
            },
        ...
        that.dropHandler = function (event, ui) {
            ...
            case 'updWord':
                ...
                View.showWordEdit( draggable.text() );
                break;
            default:
            ...
            */
            populate: function (response) {
                if (!response || !(response.hasOwnProperty('success')) || !response.success) {
                    View.dialog({title : "Retrieval Request Failed", message : response.message});
                    return;
                }
                ...
                var dragOptions = {
                    ...
                    word = null;    // temp container for Word objects

                $('#wordList').empty(); // ensures an empty #wordList
                dictionary.empty(); // ensures an empty dictionary
                if (Array.isArray(response.data)) {
                    response.data.forEach(
                        function (wordData) {
                            // create the Word objects and add them to current Dictionary
                            word = dictionary.parse( wordData );
                ...
                var totalWords = dictionary.countWords();
                ...
                View.dialog({title : "Retrieval succeeded", message : response.message});
            }
        };
    ...
    var View = (function () {
        ...
        */
        that.showWordView = function (triggerEvent, wordSpelling) {
            ...
            var word = dictionary.getWord(wordSpelling), targetDefList = null, targetSynList = null;
            View.resetWordView();
        ...
        */
        that.showWordEdit = function (sp) {
            ...
            var wordObj = (sp && dictionary.containsWord(sp)) ? dictionary.getWord(sp) : null,
                wordEdit = $('#wordEdit');
